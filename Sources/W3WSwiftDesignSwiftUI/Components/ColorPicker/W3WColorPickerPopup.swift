...
/// Anchored colour picker: the current colour sits plain in a cell that hangs off the trailing
/// edge, the whole palette fills a grid below it with the current colour marked by a border.
/// The caller positions it under the control it belongs to, dismisses it on taps outside, and
/// identifies the current colour by palette index so no colour-equality is needed here.
public struct W3WColorPickerPopup: View {
  ...
  /// - Parameters:
  ///   - currentIndex: index in `colors` of the colour shown in the attached top cell and
  ///     marked in the grid; nil or out of range shows the grid alone
  ...
  private var entries: [Entry] {
    colors.enumerated().map { Entry(index: $0.offset, color: $0.element) }
  }

  private var gridRows: [[Entry]] {
    stride(from: 0, to: entries.count, by: columns).map { Array(entries[$0 ..< min($0 + columns, entries.count)]) }
  }
  ...
  public var body: some View {
    VStack(alignment: .trailing, spacing: 0) {
      if let currentEntry {
        // the anchored copy of the current colour; the grid's copy carries the selection and the accessibility
        swatchCell(currentEntry.color, selected: false, accessibilityLabel: nil, accessibilityIdentifier: nil) {
          onPick(currentEntry.color)
        }
        .accessibility(hidden: true)   // iOS 13 spelling: the package floor predates accessibilityHidden
      }
      ForEach(Array(gridRows.enumerated()), id: \.offset) { _, row in
        HStack(spacing: 0) {
          ForEach(row, id: \.index) { entry in
            swatchCell(entry.color, selected: entry.index == currentIndex, accessibilityLabel: label(for: entry), accessibilityIdentifier: identifier(for: entry)) {
              onPick(entry.color)
            }
          }
        }
  ...
  private func swatchCell(_ color: W3WColor, selected: Bool, accessibilityLabel: String?, accessibilityIdentifier: String?, action: @escaping () -> Void) -> some View {
    Button(action: action) {
      Rectangle()
        ...
        .overlay(Rectangle().strokeBorder(theme.labelsPrimaryBlackInverse ?? .primary, lineWidth: selected ? 2 : 0))
